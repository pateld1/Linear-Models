...
**NOTE**: For the best viewing quality, view the HTML file instead of the PDF output.
...
Answer: Let the null hypothesis be that there is no linear association between crime rate and percentage of high school graduates. Let the alternative hypothesis be that there is a linear association between the two. Then at an $\alpha$ level of $.01$, the $t$ test says
```{r}
...
Answer: Let $$k_i = \frac{X_i - \overline{X})}{\sum (X_i - \overline{X})^2} $$ Then $$ \begin{aligned} 
\sum k_iX_i &= \sum \frac{X_i - \overline{X}}{\sum (X_i - \overline{X})^2} X_i \\ \sum k_iX_i + \sum \frac{\overline{X}^2 - X_i\overline{X}}{\sum (X_i - \overline{X})^2} &= \sum \frac{X_i^2 - X_i\overline{X}}{\sum (X_i - \overline{X})^2} + \sum \frac{\overline{X}^2 - X_i\overline{X}}{\sum (X_i - \overline{X})^2} \\ \sum k_iX_i + \sum \frac{\overline{X}(X_i - \overline{X})}{\sum (X_i - \overline{X})^2} &= \sum \frac{X_i^2 - 2X_i\overline{X} + \overline{X}^2}{\sum (X_i - \overline{X})^2} \\ \sum k_iX_i + 0 &= \sum \frac{(X_i - \overline{X})^2}{\sum (X_i - \overline{X})^2} \\ \sum k_iX_i &= \frac{\sum (X_i - \overline{X})^2}{\sum (X_i - \overline{X})^2} = 1 \end{aligned} $$ 


### Problem 51: 
Show that $b_0$ as defined in (2.21) is an unbiased estimator of $\beta_0$.

Answer: To show that $b_0$ is an unbiased estimator of $\beta_0$, show that $\text{E}[b_0] = \beta_0$. 
$$ \begin{aligned} \text{E}[b_0] &= \text{E}[\overline{Y} - b_1\overline{X}] \\ &= \text{E}[\overline{Y}] - \overline{X}\text{E}[b_1] \\ &= \frac{1}{n} \text{E}[Y] - \overline{X}\text{E}[b_1] \\ &= \frac{1}{n} \sum (\beta_0 + \beta_1X_i) - \overline{X}\beta_1 \\ &= \left(\frac{1}{n} \sum \beta_0 + \frac{1}{n} \sum \beta_1X_i\right) - \overline{X}\beta_1 \\ &= \frac{1}{n} n\beta_0 + \beta_1 \frac{\sum X_i}{n} - \overline{X}\beta_1 \\ &= \frac{n}{n}\beta_0 + \beta_1\overline{X} - \beta_1\overline{X} \\ &= \beta_0 \end{aligned} $$  
...
Answer: The expression in (2.22b) is $$ \sigma^2[b_0] = \sigma^2\left[\frac{1}{n} + \frac{\overline{X}^2}{\sum (X_i - \overline{X})^2}\right] $$ 
First note that $$ \sigma^2[b_0] = \sigma^2[(1)\overline{Y} - b_1\overline{X}] $$ Then 
$$ \begin{aligned} \sigma^2[b_0] &= 1^2\sigma^2[\overline{Y}] + \overline{X}^2\sigma^2[b_1] - 2(1)(\overline{X})\text{Cov}[\overline{Y}, b_1] \\ &= \sigma^2[\overline{Y}] + \overline{X}^2\sigma^2[b_1] - 2\overline{X}\text{Cov}[\overline{Y}, b_1] \\ &= \frac{\sigma^2}{n} + \overline{X}^2 \frac{\sigma^2}{\sum (X_i - \overline{X})^2} - 2\overline{X}(0) \\ &= \frac{\sigma^2}{n} + \sigma^2\frac{\overline{X}^2}{\sum (X_i - \overline{X})^2} \\ &= \sigma^2 \left( \frac{1}{n} + \frac{\overline{X}^2}{\sum (X_i - \overline{X})^2}\right) \end{aligned} $$ 
...
Answer: The conditions say:

* The conditional distributions of the $Y_i$, given $X_i$, are normal and independent, with conditional means $\beta_0 + \beta_1X_i$ and conditional variance $\sigma^2$
* The $X_i$ are independent random variables whose probability distribution $g(X_i)$ does not involve the parameters $\beta_0$, $\beta_1$ and $\sigma^2$. 

Let $Y_i = \beta_0 + \beta_1X_i$. Then the conditional distribution of $Y_i$ given $X_i$, with the above conditions, is 
$$ f(Y_i | X_i) = \frac{1}{\sqrt{2\pi \sigma^2}} \exp\left(-\frac{1}{2\sigma^2}(Y_i - \beta_0 - \beta_1X_i)^2\right) $$ 
Then the likelihood function, given that $X$ has a probability distribution of $g(X_i)$ is $$ L = \prod_i^n \frac{1}{\sqrt{2\pi \sigma^2}} \exp\left( -\frac{1}{2}\sigma^2(Y_i - \beta_0 - \beta_1X_i)^2\right) \cdot g(X_i) $$ 
...
Answer: To get the MLEs, first take the log of the likelihood function, differentiate the equation (with respect to each variable $\beta_0$, $\beta_1$ and $\sigma^2$) and set them all equal to $0$.
$$ \begin{aligned} \log L &= -\frac{n}{2} \log (2\pi \sigma^2) - \frac{1}{2\sigma^2}\sum (Y_i - \beta_0 - \beta_1X_i)^2 + \sum \log g(X_i) \\ \frac{\partial \log L}{\partial \beta_0} &= \frac{1}{\sigma^2}\sum (Y_i - \beta_0 - \beta_1X_i) = 0 \\ \frac{\partial \log L}{\partial \beta_1} &= \frac{1}{\sigma^2}\sum (Y_i - \beta_0 - \beta_1X_i) \cdot X_i = 0 \\ \frac{\partial \log L}{\partial \sigma^2} &= -\frac{n}{2}\frac{1}{\sigma^2} + \frac{1}{2}\sum (Y_i - \beta_0 - \beta_1X_i)^2 \cdot\left(\frac{1}{\sigma^2}\right)^2 = 0 \end{aligned} $$ 
The first equation simplify to:
$$ \begin{aligned} \sum Y_i - n\beta_0 - \beta_1\sum X_i &= 0 \\ \sum Y_i - nb_0 - b_1\sum X_i &= 0 \\ \sum Y_i &= nb_0 + b_1 \sum X_i \end{aligned} $$ 
This is one of the least squares normal equations.

The second equation simplify to: 
$$ \begin{aligned}  \sum Y_iX_i - \beta_0 \sum X_i - \beta_1 \sum X_i^2 &= 0 \\ \sum Y_iX_i - b_0\sum X_i - b_1\sum X_i^2 &= 0 \\ \sum Y_iX_i &= b_0\sum X_i + b_1\sum X_i^2 \end{aligned} $$ 
This is the other least squares normal equation.

Finally, $$ \begin{aligned} 
\frac{1}{2} \sum (Y_i - \beta_0 - \beta_1X_i)^2 \cdot \left( \frac{1}{\sigma^2}\right)^2 &= \frac{n}{2\sigma^2} \\ \sigma^2 &= \frac{\sum (Y_i - \beta_0 - \beta_1X_i)^2}{n} \\ &= \frac{\sum (Y_i - b_0 - b_1X_i)^2}{n} \end{aligned} $$ 

All three of these MLEs are the same as the ones in (1.27) when the $X_i$ are fixed.
...
Answer: Under this scenario, $\beta_1 = 0$ would still indicate that there is no linear association between $X$ and $Y$ because the association does not take into account of the magnitude of the variance. But it cannot claim that there is no association between $X$ and $Y$ because it is possible the association is nonlinear. 
...
$$ \begin{aligned} \text{SSR} &= \sum (\hat{Y}_i - \overline{Y})^2 \\ &= \sum ((b_0 - b_1X_i) - \overline{Y})^2 \\ &= \sum ((\overline{Y} - b_1\overline{X}) + b_1X_i - \overline{Y})^2 \\ &= \sum (\overline{Y} - b_1\overline{X} + b_1\overline{X} - \overline{Y})^2 \\ &= \sum (b_1X_i - b_1\overline{X})^2 \\ &= b_1^2 \sum (X_i - \overline{X})^2 \end{aligned} $$ 


### Problem 56: 
In a small-scale regression study, five observations on $Y$ were obtained corresponding to $X=1$, $4$, $10$, $11$ and $14$. Assume that $\sigma = .6$, $\beta_0=5$ and $\beta_1 = 3$.

(a) What are the expected values of MSR and MSE here? 

Answer: The expected value of MSR is $$ \text{E[MSR]} = \sigma^2 + \beta_1^2 \sum (X_i - \overline{X})^2 $$ or
```{r}
x = c(1, 4, 10, 11, 14)
sigma = .6
beta0 = 5
...
sigma^2 + (beta1^2 * sum((x - mean(x))^2))
```

Similarly, the expected value of MSE is $$ \text{E[MSE]} = \sigma^2 $$ 
or 
```{r}
sigma^2
```
...
Answer: Using these $X_i$ values, the expected value of the MSR should be different from above since all the $X_i$s are different. The expected value of MSE should remain the same since it only depends on $\sigma$. 
...
and the expected value of MSE is
```{r}
sigma^2
```
...
Answer: The reduced model is $$ Y_i - 5 = \beta_0 + \varepsilon_i $$ and the degrees of freedom $\text{df}_R$ is $n-1$. 
...
Answer: The reduced model is $$ Y_i - 5X_i - 2 = \varepsilon_i $$ and the degrees of freedom $\text{df}_R$ is $n$. 
...
Answer: If $\rho_{12}$, then 
$$ \begin{aligned} 
f(Y_1, Y_2) &= \frac{1}{2\pi\sigma_1\sigma_2 \sqrt{1 - \rho^2_{12}}} \exp\left(-\frac{1}{2(1 - \rho^2_{12})} \left[\left(\frac{Y_i - \mu_1}{\sigma_1}\right)^2 - 2\rho_{12}\left(\frac{Y_1 - \mu_1}{\sigma_1}\right)\left(\frac{Y_2 - \mu_2}{\sigma_2}\right) + \left(\frac{Y_2 - \mu_2}{\sigma_2}\right)^2\right]\right) \\ &= \frac{1}{2\pi\sigma_1\sigma_2 \sqrt{1 - 0}} \exp\left(-\frac{1}{2(1 - 0)} \left[\left(\frac{Y_i - \mu_1}{\sigma_1}\right)^2 - 2 \cdot 0 \cdot \left(\frac{Y_1 - \mu_1}{\sigma_1}\right)\left(\frac{Y_2 - \mu_2}{\sigma_2}\right) + \left(\frac{Y_2 - \mu_2}{\sigma_2}\right)^2\right]\right) \\ &= \frac{1}{2\pi\sigma_1\sigma_2}\exp\left(-\frac{1}{2} \left[\left(\frac{Y_1-\mu_1}{\sigma_1}\right)^2 + \left(\frac{Y_2-\mu_2}{\sigma_2}\right)^2\right]\right) \\ &= \left(\frac{1}{\sqrt{2\pi}\sigma_1}\exp\left(-\frac{1}{2}\left(\frac{Y_1-\mu_1}{\sigma_1}\right)^2\right)\right) \cdot \left(\frac{1}{\sqrt{2\pi}\sigma_2}\exp\left(-\frac{1}{2}\left(\frac{Y_2-\mu_2}{\sigma_2}\right)^2\right)\right) \\ &= f_1(Y_1) \cdot f_2(Y_2) \end{aligned} $$ 
...
Answer: The probability density function of the bivariate normal distribution is given in the last problem. Then its likelihood function is $$ L = \prod \frac{1}{2\pi \sigma_1\sigma_2 \sqrt{1-\rho^2_{12}}} \exp\left( -\frac{1}{2(1 - \rho^2_{12})} \left( \left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)^2 - 2\rho_{12}\left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)\left(\frac{Y_{2i} - \mu_2}{\sigma_2}\right) + \left(\frac{Y_{2i} - \mu_2}{\sigma_2}\right)^2\right)\right) $$ 

Take the $\log$ of this function. 
$$ \log L = -n\log(2\pi) - n\log(\sigma_1) - n\log(\sigma_2) - \frac{n}{2}\log(1-\rho^2_{12}) - \frac{1}{2(1-\rho^2_{12})} \sum \left( \left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)^2 - 2\rho_{12}\left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)\left(\frac{Y_{2i} - \mu_2}{\sigma_2}\right) + \left(\frac{Y_{2i}-\mu_2}{\sigma_2}\right)^2\right) $$ 

Differentiate this function with respect to $\mu_1$, $\sigma_1$, $\mu_2$, $\sigma_2$ and $\rho_{12}$: 
$$ \begin{aligned} 
\frac{\partial \log L}{\partial \mu_1} &= -\frac{1}{2(1-\rho_{12}^2)} \sum \left( \frac{-2(Y_{1i} - \mu_1)}{\sigma_1^2} + \frac{2\rho_{12}(Y_{2i} - \mu_2)}{\sigma_1\sigma_2}\right) \\
\frac{\partial \log L}{\partial \sigma_1} &= -\frac{n}{\sigma_1} + \frac{1}{2(1-\rho_{12}^2)} \left(\frac{ 2\sum (Y_{1i} - \mu_1)^2}{\sigma_1^3} - \frac{2\rho_{12} \sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1^2\sigma_2}\right) \\
\frac{\partial \log L}{\partial \mu_2} &= -\frac{1}{2(1-\rho_{12}^2)} \sum \left( \frac{-2(Y_{2i} - \mu_2)}{\sigma_2^2} + \frac{2\rho_{12}(Y_{1i} - \mu_1)}{\sigma_1\sigma_2}\right) \\
\frac{\partial \log L}{\partial \sigma_2} &= -\frac{n}{\sigma_2} + \frac{1}{2(1 - \rho_{12}^2)}\left(\frac{2 \sum (Y_{2i} - \mu_2)^2}{\sigma_2^3} - \frac{2\rho_{12}\sum (Y_{1i} - \mu_1)(Y_{2i}-\mu_2)}{\sigma_1\sigma_2^2}\right) \\ 
\frac{\partial \log L}{\partial \rho_{12}} &= \left(-\frac{n}{2} \cdot \frac{1}{1-\rho_{12}^2} \cdot -2\rho_{12}\right) + \left(-\frac{1}{2(1-\rho_{12}^2)} \cdot -2\left(\sum \frac{(Y_{1i} - \mu_1)}{\sigma_1} \frac{(Y_{2i} - \mu_2)}{\sigma_2}\right)\right) \\ &+ \left(-\frac{1}{2} \cdot \frac{2\rho_{12}}{(1-\rho_{12}^2)^2} \sum \left( \left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)^2 - 2\rho_{12}\left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)\left(\frac{Y_{2i} - \mu_2}{\sigma_2}\right) + \left(\frac{Y_{2i} - \mu_2}{\sigma_2}\right)^2 \right)\right)
\end{aligned} $$

Note first that 
$$ \rho_{12} = \frac{\text{Cov}[Y_1, Y_2]}{\sigma_1\sigma_2} = \frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{n\sigma_1\sigma_2} \to n\rho_{12} = \frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{n\sigma_1\sigma_2} $$ 

Now if the derivative for $\frac{\partial \log L}{\partial \mu_1} = 0$, then 
$$ \begin{aligned}  -\frac{1}{2(1-\rho_{12}^2)} \sum \left( \frac{-2(Y_{1i} - \mu_1)}{\sigma_1^2} + \frac{2\rho_{12}(Y_{2i} - \mu_2)}{\sigma_1\sigma_2}\right) &= 0 \\
\frac{2\sum (Y_{1i} - \mu_1)}{2(1-\rho_{12}^2)\sigma_1^2} - \frac{2\rho_{12} \sum (Y_{2i} - \mu_2)}{2 \sigma_1\sigma_2 (1-\rho_{12}^2)} &= 0 \\ \frac{\sum (Y_{1i} - \mu_1)}{\sigma_1^2(1-\rho_{12}^2)} - \frac{\rho_{12}\sum (Y_{2i} - \mu_2)}{\sigma_1\sigma_2(1-\rho_{12}^2)} &= 0 \\ \frac{1}{1-\rho_{12}^2}\left( \frac{\sum (Y_{1i}-\mu_1)}{\sigma_1^2} - \frac{\rho_{12}\sum (Y_{2i} - \mu_2)}{\sigma_1\sigma_2}\right) &= 0 \\ \frac{\sum (Y_{1i} - \mu_1)}{\sigma_1^2} - \frac{\rho_{12}\sum (Y_{2i} - \mu_2)}{\sigma_1\sigma_2} &= 0 \\ \sum (Y_{1i} - \mu_1) - \frac{\rho_{12} \sigma_1 \sum (Y_{2i} - \mu_2)}{\sigma_2} &= 0 \\ \sum (Y_{1i} - \mu_1) - \frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{n\sigma_1\sigma_2} \cdot \frac{\sigma_1 \sum (Y_{2i} - \mu_2)}{\sigma_2} &= 0 \\ \sum (Y_{1i} - \mu_1) - \frac{\sum (Y_{1i} - \mu_1) \sum (Y_{2i} - \mu_2)^2}{n\sigma_2^2} &= 0 \\ \sum (Y_{1i} - \mu_1) - \frac{n\sigma_2^2\sum (Y_{1i} - \mu_1)}{n\sigma_2^2} &= 0 \\ \sum (Y_{1i} - \mu_1) - \sum (Y_{1i} - \mu_1) &= 0 \\ \sum (Y_{1i} - \mu_1) &= \sum (Y_{1i} - \mu_1) \\ n(\overline{Y}_1 - \mu_1) &= n(\overline{Y}_1 - \mu_1) \\ \mu_1 &= \overline{Y}_1 \\ \hat{\mu}_1 &= \overline{Y}_1 \end{aligned} $$ 

Similarly, if the derivative for $\frac{\partial \log L}{\partial \mu_2} = 0$, then
$$ \begin{aligned}  -\frac{1}{2(1-\rho_{12}^2)} \sum \left( \frac{-2(Y_{2i} - \mu_2)}{\sigma_2^2} + \frac{2\rho_{12}(Y_{1i} - \mu_1)}{\sigma_1\sigma_2}\right) &= 0 \\
\frac{2\sum (Y_{2i} - \mu_2)}{2(1-\rho_{12}^2)\sigma_2^2} - \frac{2\rho_{12} \sum (Y_{1i} - \mu_1)}{2 \sigma_1\sigma_2 (1-\rho_{12}^2)} &= 0 \\ \frac{\sum (Y_{2i} - \mu_2)}{\sigma_2^2(1-\rho_{12}^2)} - \frac{\rho_{12}\sum (Y_{1i} - \mu_1)}{\sigma_1\sigma_2(1-\rho_{12}^2)} &= 0 \\ \frac{1}{1-\rho_{12}^2}\left( \frac{\sum (Y_{2i}-\mu_2)}{\sigma_2^2} - \frac{\rho_{12}\sum (Y_{1i} - \mu_1)}{\sigma_1\sigma_2}\right) &= 0 \\ \frac{\sum (Y_{2i} - \mu_2)}{\sigma_2^2} - \frac{\rho_{12}\sum (Y_{1i} - \mu_1)}{\sigma_1\sigma_2} &= 0 \\ \sum (Y_{2i} - \mu_2) - \frac{\rho_{12} \sigma_2 \sum (Y_{1i} - \mu_1)}{\sigma_1} &= 0 \\ \sum (Y_{2i} - \mu_2) - \frac{\sum (Y_{2i} - \mu_2)(Y_{1i} - \mu_1)}{n\sigma_1\sigma_2} \cdot \frac{\sigma_2 \sum (Y_{1i} - \mu_1)}{\sigma_1} &= 0 \\ \sum (Y_{2i} - \mu_2) - \frac{\sum (Y_{2i} - \mu_2) \sum (Y_{1i} - \mu_1)^2}{n\sigma_1^2} &= 0 \\ \sum (Y_{2i} - \mu_2) - \frac{n\sigma_1^2\sum (Y_{2i} - \mu_2)}{n\sigma_1^2} &= 0 \\ \sum (Y_{2i} - \mu_2) - \sum (Y_{2i} - \mu_2) &= 0 \\ \sum (Y_{2i} - \mu_2) &= \sum (Y_{2i} - \mu_2) \\ n(\overline{Y}_2 - \mu_2) &= n(\overline{Y}_2 - \mu_2) \\ \mu_2 &= \overline{Y}_2 \\ \hat{\mu}_2 &= \overline{Y}_2 \end{aligned} $$ 

Thus the MLE for $\mu_1$ and $\mu_2$ are $$ \begin{aligned} \hat{\mu}_1 &= \overline{Y}_1 \\ \hat{\mu}_2 &= \overline{Y}_2 \end{aligned} $$ 

Now if the derivative for $\frac{\partial \log L}{\partial \sigma_1} = 0$, and with the above results, then

$$ \begin{aligned} -\frac{n}{\sigma_1} + \frac{1}{2(1-\rho_{12}^2)} \left(\frac{ 2\sum (Y_{1i} - \mu_1)^2}{\sigma_1^3} - \frac{2\rho_{12} \sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1^2\sigma_2}\right) &= 0 \\ -\frac{n}{\sigma_1} + \frac{1}{2(1-\rho_{12}^2)} \left(\frac{2\sum (Y_{1i} - \mu_1)^2}{\sigma_1^3} - \frac{2n\rho_{12}^2}{\sigma_1} \right) &= 0 \\ -\frac{n}{\sigma_1} + \frac{1}{1-\rho^2_{12}} \left(\frac{\sum (Y_{1i} - \mu_1)^2}{\sigma_1^3} - \frac{n\rho_{12}^2}{\sigma_1} \right) &= 0 \\ 
-n + \frac{1}{1 - \rho^2_{12}}\left( \frac{ \sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} - n\rho^2_{12} \right) &= 0 \\ \frac{1}{1 - \rho^2_{12}}\left( \frac{ \sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} - n\rho^2_{12} \right) &= n \\ \frac{\sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} - n\rho^2_{12} &= n(1-\rho^2_{12}) = n - n\rho^2_{12} \\ 
\frac{\sum (Y_{1i} - \mu_1)}{\sigma_1^2} - n\rho^2_{12} -n + n\rho_{12}^2 &= 0 \\ \frac{\sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} - n &= 0 \\ \frac{\sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} &= n \\ \frac{\sum (Y_{1i} - \mu_1)^2}{n} &= \sigma_1^2 \\ \sqrt{\frac{\sum (Y_{1i} - \mu_1)^2}{n}} &= \sigma_1 \\ \sqrt{\frac{\sum (Y_{1i} - \overline{Y}_1)^2}{n}} &= \hat{\sigma}_1 \end{aligned} $$ 

Similarly, if the derivative for $\frac{\partial \log L}{\partial \sigma_2} = 0$, and with the above results, then 

$$ \begin{aligned} -\frac{n}{\sigma_2} + \frac{1}{2(1-\rho_{12}^2)} \left(\frac{ 2\sum (Y_{2i} - \mu_2)^2}{\sigma_2^3} - \frac{2\rho_{12} \sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2^2}\right) &= 0 \\ -\frac{n}{\sigma_2} + \frac{1}{2(1-\rho_{12}^2)} \left(\frac{2\sum (Y_{2i} - \mu_2)^2}{\sigma_2^3} - \frac{2n\rho_{12}^2}{\sigma_2} \right) &= 0 \\ -\frac{n}{\sigma_2} + \frac{1}{1-\rho^2_{12}} \left(\frac{\sum (Y_{2i} - \mu_2)^2}{\sigma_2^3} - \frac{n\rho_{12}^2}{\sigma_2} \right) &= 0 \\ 
-n + \frac{1}{1 - \rho^2_{12}}\left( \frac{ \sum (Y_{2i} - \mu_2)^2}{\sigma_2^2} - n\rho^2_{12} \right) &= 0 \\ \frac{1}{1 - \rho^2_{12}}\left( \frac{ \sum (Y_{2i} - \mu_2)^2}{\sigma_2^2} - n\rho^2_{12} \right) &= n \\ \frac{\sum (Y_{2i} - \mu_2)^2}{\sigma_2^2} - n\rho^2_{12} &= n(1-\rho^2_{12}) = n - n\rho^2_{12} \\ 
\frac{\sum (Y_{2i} - \mu_2)}{\sigma_2^2} - n\rho^2_{12} -n + n\rho_{12}^2 &= 0 \\ \frac{\sum (Y_{2i} - \mu_2)^2}{\sigma_2^2} - n &= 0 \\ \frac{\sum (Y_{2i} - \mu_2)^2}{\sigma_2^2} &= n \\ \frac{\sum (Y_{2i} - \mu_2)^2}{n} &= \sigma_2^2 \\ \sqrt{\frac{\sum (Y_{2i} - \mu_2)^2}{n}} &= \sigma_2 \\ \sqrt{\frac{\sum (Y_{2i} - \overline{Y}_2)^2}{n}} &= \hat{\sigma}_2 \end{aligned} $$ 

The MLE for $\sigma_1$ and $\sigma_2$ are $$ \begin{aligned} \hat{\sigma}_1 &= \sqrt{\frac{\sum (Y_{1i} - \overline{Y}_1)^2}{n}} \\ \hat{\sigma}_2 &= \sqrt{ \frac{ \sum (Y_{2i} - \overline{Y}_2)^2}{n}} \end{aligned} $$ 

If the derivative for $\frac{\partial \log L}{\partial \rho_{12}} = 0$, then

$$ \begin{aligned} \left(-\frac{n}{2} \cdot \frac{1}{1-\rho_{12}^2} \cdot -2\rho_{12}\right) + \left(-\frac{1}{2(1-\rho_{12}^2)} \cdot -2\left(\sum \frac{(Y_{1i} - \mu_1)}{\sigma_1}\frac{(Y_{2i} - \mu_2)}{\sigma_2}\right)\right) + \left(-\frac{1}{2} \cdot \frac{2\rho_{12}}{(1-\rho_{12}^2)^2} \sum \left( \left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)^2 - 2\rho_{12}\left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)\left(\frac{Y_{2i} - \mu_2}{\sigma_2}\right) + \left(\frac{Y_{2i} - \mu_2}{\sigma_2}\right)^2 \right)\right) &= 0 \\ \frac{n\rho_{12}}{1-\rho_{12}^2} + \frac{1}{1-\rho_{12}^2} \frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} - \frac{\rho_{12}}{(1-\rho_{12}^2)^2} \sum \left( \left( \frac{Y_{1i} - \mu_1)}{\sigma_1}\right)^2 - 2\rho_{12}\left(\frac{Y_{1i} - \mu_1}{\sigma_1}\right)\left(\frac{Y_{2i}-\mu_2}{\sigma_2}\right) + \left(\frac{Y_{2i}-\mu_2}{\sigma_2}\right)^2\right) &= 0 \\ \frac{n\rho_{12}}{1-\rho_{12}^2} + \frac{1}{1-\rho_{12}^2}\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} - \frac{\rho_{12}}{(1-\rho_{12}^2)^2}\frac{\sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} + \frac{2\rho_{12}^2}{(1-\rho_{12})^2}\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} - \frac{\rho_{12}}{(1-\rho_{12}^2)^2}\frac{\sum (Y_{2i} - \mu_2)^2}{\sigma_2^2} &= 0 \\ n\rho_{12}(1-\rho_{12}^2) + (1-\rho_{12}^2)\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} - \rho_{12}\frac{\sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} + 2\rho_{12}^2\frac{\sum (Y_{1i} - \mu_1)(Y_{2i}-\mu_2)}{\sigma_1\sigma_2} - \rho_{12}\frac{\sum (Y_{2i} - \mu_2)^2}{\sigma_2^2} &= 0 \\ n\rho_{12}(1-\rho_{12}^2) + (1 + \rho_{12}^2)\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} - \rho_{12}\frac{\sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} - \rho_{12}\frac{\sum (Y_{2i} - \mu_2)^2}{\sigma_2^2} &= 0 \\ n\rho_{12}(1-\rho_{12}^2) + (1+\rho_{12}^2)\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} - \rho_{12}\left(\frac{\sum (Y_{1i} - \mu_1)^2}{\sigma_1^2} + \frac{\sum (Y_{2i} - \mu_2)^2}{\sigma_2^2}\right) &= 0 \\ n\rho_{12}(1-\rho_{12}^2) + (1+\rho_{12}^2)\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} - 2n\rho_{12} &= 0 \\ n\rho_{12} - n\rho_{12}^3 + (1+\rho_{12}^2)\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} - 2n\rho_{12} &= 0 \\ -n\rho_{12} - n\rho_{12}^3 + (1 + \rho_{12}^2)\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} &= 0 \\ -n\rho_{12} - n\rho_{12}^3 + \frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} + \rho_{12}^2\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} &= 0 \\ -n\rho_{12} - n\rho_{12}^3 + n\rho_{12} + \rho_{12}^2\frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sigma_1\sigma_2} &= 0 \\ -n\rho_{12}^3 + \rho_{12}^2\frac{\sum (Y_{1i} - \mu_1)(Y_{2i}-\mu_2)}{\sigma_1\sigma_2} &= 0 \\ \rho_{12}^2\frac{\sum (Y_{1i}-\mu_1)(Y_{2i}-\mu_2)}{\sigma_1\sigma_2} &= n\rho_{12}^3 \\ \frac{\sum (Y_{1i} - \mu_1)(Y_{2i}-\mu_1)}{\sigma_1\sigma_2} &= n\rho_{12} \\ \frac{\sum (Y_{1i} - \mu_1)(Y_{2i}-\mu_2)}{n\sigma_1\sigma_2} &= \rho_{12} \\ \frac{\sum (Y_{1i} - \mu_1)(Y_{2i} - \mu_2)}{\sum (Y_{1i} - \mu_1)\sum(Y_{2i} - \mu_2)} &= \rho_{12} \\ \frac{\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2)}{\sum (Y_{1i} - \overline{Y}_1)\sum(Y_{2i} - \overline{Y}_2)} &= \hat{\rho}_{12} \end{aligned} $$ 

The MLE for $\rho_{12}$ is $$ \hat{\rho}_{12} = \frac{\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2)}{ \sum (Y_{1i} - \overline{Y}_1)(\sum (Y_{2i} - \overline{Y}_2)} $$ 
...
Answer: In (2.80), the following is given, $$ \begin{aligned} 
\alpha_{1|2} &= \mu_1 - \mu_2\rho_{12}\frac{\sigma_1}{\sigma_2} \\ 
\beta_{12} &= \rho_{12} \frac{\sigma_1}{2} \\ 
\sigma^2_{1|2} &= \sigma^2_1(1 - \rho^2_{12}) \end{aligned} $$ 

Using the results from above, the MLEs are subsituted to form the following: 
$$ \begin{aligned} 
\hat{\alpha}_{1|2} &= \hat{\mu}_1 - \hat{\mu}_2\hat{\rho}_{12}\frac{\hat{\sigma}_1}{\hat{\sigma}_2} \\ 
&= \hat{Y}_1 - \hat{Y}_2\left( \frac{ \sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2)}{\left( \sum (Y_{1i} - \overline{Y}_1)^2\right)^\frac{1}{2}}\right) \left( \frac{\sqrt{ \frac{1}{n} \sum (Y_{1i} - \overline{Y}_1})^2}{\sqrt{ \frac{1}{n} \sum (Y_{2i} - \overline{Y}_2})^2}\right) \\
&= \overline{Y}_1 - \overline{Y}_2 \left( \frac{\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2)}{\sum (Y_{2i} - \overline{Y}_2)^2}\right) \\
\hat{\beta}_{12} &= \hat{\rho}_{12} \frac{\hat{\sigma}_1}{\hat{\sigma}_2} \\ 
&= \left( \frac{\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2)}{(\sum (Y_{1i} - \overline{Y}_1)^2)^{\frac{1}{2}}(\sum (Y_{2i} - \overline{Y}_2)^2)^{\frac{1}{2}}}\right) \left( \frac{\sqrt{ \frac{1}{n} \sum (Y_{1i} - \overline{Y}_1)^2}}{\sqrt{\frac{1}{n} \sum (Y_{2i} - \overline{Y}_2)^2}}\right) \\ &= \frac{\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2)}{\sum (Y_{2i} - \overline{Y}_2)^2} \\ 
\hat{\sigma}^2_{1|2} &= \hat{\sigma}^2_1(1 - \hat{\rho}^2_{12}) \\ 
&= \frac{\sum (Y_{1i} - \overline{Y}_1)^2}{n} \left( 1 - \frac{(\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2))^2}{\sum (Y_{1i} - \overline{Y}_1)^2 \sum(Y_{2i} - \overline{Y}_2)^2}\right) \\ 
&= \frac{\sum (Y_{1i} - \overline{Y}_1)^2}{n} - \frac{(\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2))^2}{n \sum (Y_{2i} - \overline{Y}_2)^2} \end{aligned} $$ 
...
Answer: Let $X_i = Y_{2i}$ and $Y_i = Y_{1i}$. Then
$$ \begin{aligned} 
\hat{\alpha}_{1|2} &= \overline{Y}_1 - \overline{Y}_2 \left( \frac{\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2)}{\sum (Y_{2i} - \overline{Y}_2)^2}\right) \\ &= \overline{Y} - \overline{X}\left(\frac{\sum (Y_i - \overline{Y})(X_i - \overline{X})}{\sum (X_i - \overline{X})^2} \right) \\
\hat{\beta}_{12} &= \frac{\sum (Y_{1i} - \overline{Y}_1)(Y_{2i} - \overline{Y}_2)}{\sum (Y_{2i} - \overline{Y}_2)^2} \\ &= \frac{\sum (Y_i - \overline{Y})(X_i - \overline{X})}{\sum (X_i - \overline{X})^2} \end{aligned} $$ 

These results are the same as the least squares estimators, $\hat{\beta}_0$ and $\hat{\beta}_1$, for the regression coefficients in the simple linear regression model. 

### Problem 60: 
Show that test statistic (2.17) and (2.87) are equivalent.

Answer: The test statistic in (2.17) is $$ t^* = \frac{b_1}{\text{s}[b_1]}$$ and the test statistic in (2.87) is $$ t^* = \frac{r_{12}\sqrt{n-2}}{\sqrt{1-r^2_{12}}} $$ 

Note the following properties: $$ \begin{aligned} b_1 &= \frac{\sum (X_i - \overline{X})(Y_i - \overline{Y})}{\sum (X_i - \overline{X})^2} \\ \text{SSR} &= b_1^2 \sum (X_i - \overline{X})^2 \end{aligned} $$ 

Begin with the equation for SSTO. 
$$ \begin{aligned} 
\text{SSTO} &= \text{SSR} + \text{SSE} \\ \text{SSE} &= \text{SSTO} - \text{SSR} \\ &= \sum (Y_i - \overline{Y})^2 - b_1^2 \sum (X_i - \overline{X})^2 \\ &= \sum (Y_i - \overline{Y})^2 - \left(\frac{\sum (X_i - \overline{X})(Y_i - \overline{Y})}{\sum (X_i - \overline{X})^2}\right)^2 \sum (X_i - \overline{X})^2 \\ &= \sum (Y_i - \overline{Y})^2 - \frac{\left( \sum (X_i - \overline{X})(Y_i - \overline{Y})\right)^2}{\sum (X_i - \overline{X})^2}
\end{aligned} $$ 

To break this apart, note that $$ \begin{aligned} s_x^2 &= \frac{\sum (X_i - \overline{X})^2}{n-1} &\to \sum (X_i - \overline{X})^2 &= (n-1)s_x^2 \\ s_y^2 &= \frac{\sum (Y_i - \overline{Y})^2}{n-1} &\to \sum (Y_i - \overline{Y})^2 &= (n-1)s_y^2 \end{aligned} $$ 

Now, if $X = Y_{i1}$ and $Y = Y_{i2}$, then $$ r_{12} = \frac{ \sum (X - \overline{X})(Y - \overline{Y})}{\left( \sum (X - \overline{X})^2 \sum (Y - \overline{Y})^2\right)^\frac{1}{2}} $$ 
and if
$$ b_1 = r_{12}\frac{s_y}{s_x} = r_{12}\left( \frac{\sum (Y_i - \overline{Y})^2}{\sum (X_i - \overline{X})^2}\right)^\frac{1}{2} $$ 

Then $$ \begin{aligned} 
\text{SSE} &= \sum (Y_i - \overline{Y})^2 - \frac{\left( \sum (X_i - \overline{X})(Y_i - \overline{Y})\right)^2}{\sum (X_i - \overline{X})^2} \\ &= \sum (Y_i - \overline{Y})^2 - \left(\frac{ \sum (X - \overline{X})(Y - \overline{Y})}{\left( \sum (X - \overline{X})^2 \sum (Y - \overline{Y})^2\right)^\frac{1}{2}}\right)^2 \cdot \sum (Y_i - \overline{Y})^2 \\ &= (n-1)s_y^2 - r^2_{12}(n-1)s_y^2 \\ &= (n-1)s_y^2(1 - r^2_{12}) 
\end{aligned} $$ 

Use this result in the standard deviation of $b_1$.
$$ \begin{aligned} \text{s}^2[b_1] &= \frac{\text{MSE}}{\sum (X_i - \overline{X})^2} \\ &= \frac{\frac{\text{SSE}}{n-2}}{\sum (X_i - \overline{X})^2} \\ &= \frac{\frac{(n-1)s_y^2(1 - r^2_{12}) }{n-2}}{\sum (X_i - \overline{X})^2} \\ &=  \frac{\frac{(n-1)s_y^2(1 - r^2_{12}) }{n-2}}{(n-1)s_x^2} \\ &= \frac{(n-1)s_y^2(1-r^2_{12})}{(n-1)(n-2)s_x^2} \\ &= \frac{1-r^2_{12}}{n-2}\frac{s_y^2}{s_x^2} \end{aligned} $$ 

Thus
$$ \begin{aligned} \frac{b_1}{\text{s}[b_1]} &= r_{12}\frac{s_y}{s_x} \cdot \frac{s_x\sqrt{n-2}}{s_y\sqrt{1-r^2_{12}}} \\ &= \frac{r_{12}\sqrt{n-2}}{\sqrt{1-r^2_{12}}} \\ &= t^*
\end{aligned} $$ 
...
Answer: If $Y_1$ is regressed on $Y_2$, then $$ \begin{aligned} \frac{\text{SSR}(Y_1)}{\text{SSTO}} &= \frac{ \left(\frac{\sum (Y_{1i} - \overline{Y}_1)(Y_{2i}-\overline{Y}_2)}{\sum (Y_{1i} - \overline{Y}_1)^2}\right)^2 \left( \sum (Y_{1i} - \overline{Y}_1)^2\right)}{\sum (Y_{2i} - \overline{Y}_2)^2} \\ &= \frac{\left(\sum (Y_{1i} - \overline{Y}_1)(Y_{2i}-\overline{Y}_2)\right)^2}{\sum (Y_{1i} - \overline{Y}_1)^2 \sum (Y_{2i} - \overline{Y}_2)^2} \end{aligned} $$ 
Similarly, $$ \begin{aligned} \frac{\text{SSR}(Y_2)}{\text{SSTO}} &= \frac{\left(\frac{\sum (Y_{2i} - \overline{Y}_2)(Y_{1i} - \overline{Y}_1)}{\sum (Y_{2i} - \overline{Y}_2)}\right)^2 \left(\sum (Y_{2i} - \overline{Y}_2)\right)^2}{\sum (Y_{1i} - \overline{Y}_1)^2} \\ &= \frac{\left(\sum (Y_{2i} - \overline{Y}_2)(Y_{1i} - \overline{Y}_1)\right)^2}{\sum (Y_{1i} - \overline{Y}_1)^2 \sum (Y_{2i} - \overline{Y}_2)^2} \end{aligned} $$ These two quantites are equal in nature where the term in the numerator is swapped in both equations. 
...
data.frame("var" = colnames(cdi_new), "r2" = r2_list) %>% arrange(desc(r2))
```
...
for(i in 1:4){
  cdi_temp = cdi_geo %>% filter(geographic_region == i)  %>% 
    select(x = perc_bach, y = per_capita_income)
  model = lm.fit_manual(cdi_temp$x, cdi_temp$y)
  conf_int = b1_conf_interval(cdi_temp, alpha = .1)
  
  beta0 = c(beta0, model[1])
  beta1 = c(beta1, model[2])
...
cdi %>% ggplot(aes(x = perc_bach, y = per_capita_income, color = geographic_region)) + 
  geom_point() + 
  scale_color_manual(values = wes_palette("IsleofDogs1", 4)) + 
  facet_grid(~ geographic_region) + 
  geom_abline(data = coefs, aes(slope = beta1, intercept = beta0), linetype = "dashed") + 
  labs(x = "% of People with a Bachelors Degree",
       y = "Per Capita Income", 
       title = "Predicting Per Capita Income using Education") + 
  theme_minimal() + 
  theme(legend.position = "bottom")
```

### Problem 64: 
Refer to the *SENIC* data set in Appendix C.1 and Project 1.45. Using $R^2$ as the criterion, which predictor variable accounts for the largest reduction in the variability of the average length of stay? 
...
senic = read.csv('APPENC01.txt', sep = '', header = FALSE, 
                  ...
                  colClasses = senic_colclasses) %>% select(-ID)
senic_new = senic %>% select(-length_stay)
r2_list = c()
for(i in 1:ncol(senic_new)){
  data = data.frame(cbind(x = senic_new[[i]], y = senic$length_stay))
  model = lm.fit_manual(data$x, data$y)
  preds = model[1] + model[2]*data$x
  r2 = R2_manual(SSR(data), SSTO(data))
  r2_list = c(r2_list, as.numeric(substr(r2, 1, nchar(r2)-1)))
}
data.frame("var" = colnames(senic_new), "r2" = r2_list) %>% arrange(desc(r2))

```

The predictor variable that accounts for the largest reduction in the variability of the average length of stay is infection rate.
...
Answer: 
```{r}
senic_geo = senic %>% select(infection_risk, length_stay, region)
lower = c()
beta0 = c()
beta1 = c()
upper = c()
...
for(i in 1:4){
  senic_temp = senic_geo %>% filter(region == i)  %>% 
    select(x = infection_risk, y = length_stay)
  model = lm.fit_manual(senic_temp$x, senic_temp$y)
  conf_int = b1_conf_interval(senic_temp, alpha = .1)
...
coefs = data.frame(region = 1:4, lower, beta0, beta1, upper)
coefs
```

The regression lines for the different regions do not have similar slopes. 
```{r}
senic %>% ggplot(aes(x = infection_risk, y = length_stay, color = factor(region))) + 
  geom_point() + 
  scale_color_manual(values = wes_palette("IsleofDogs1", 4), name = "Region") + 
  facet_grid(~ region) + 
  geom_abline(data = coefs, aes(slope = beta1, intercept = beta0), linetype = "dashed") + 
  labs(x = "Infection Risk",
       y = "Length of Stay", 
       title = "Predicting Length of Stay in Hospital using Infection Risk") + 
  theme_minimal() + 
  theme(legend.position = "bottom")
```
...
set.seed(2020)
X = seq(4, 20, by = 4)
error = rnorm(5, mean = 0, sd = sqrt(25))
Y = 20 + (4*X) + error
```

The least squares estimators $b_0$ and $b_1$, respectively, are
```{r}
model = lm.fit_manual(X, Y)
model
```

$\hat{Y}_h$ when $X_h = 10$ is
```{r}
model[1] + model[2]*10
```

The $95\%$ confidence interval for $\text{E}[Y_h]$ when $X_h = 10$ is
```{r}
Yhat_conf_interval(data.frame(x=X, y=Y), Xhat = 10, alpha = .05)
```

(b) Repeat part (a) $200$ times, generating random numbers each time.

Answer: For the next part of this question, retain the $b_0$ and $b_1$ estimates as well as the $95\%$ confidence bounds for $\text{E}[Y_h]$ when $X_h=10$.
```{r}
b0 = c()
b1 = c()
cb_lower = c()
...
for(i in 1:n){
  ...
  b0 = c(b0, model[1])
  b1 = c(b1, model[2])
  Yhat_conf_int = Yhat_conf_interval(data.frame(x=X, y=Y), Xhat = 10, alpha = .05)
  cb_lower = c(cb_lower, Yhat_conf_int[1])
  cb_upper = c(cb_upper, Yhat_conf_int[2])
}
```
...
Answer: The frequency distribution of the $200$ estimates of $b_1$ is as seen below.
```{r}
ggplot(data.frame(x=b1), aes(x=x)) + 
  geom_histogram(bins = 25, color = "black", fill = "darkgreen") + 
  labs(x = "b1 estimate", y = "count", title = "Frequency Distribution of b1 Estimates") + 
  theme_minimal()
```

The mean and standard deviations of the $b_1$ estimates, respectively, are
```{r}
mean(b1)
sd(b1)
```

The mean estimate is consistent with the given value of $\beta_1$. The standard deviation is close to the standar deviation of the error terms. 
...
Answer: When $X_h = 10$, $Y_h = 60$. The proportion of confidence intervals containing $60$ is
```{r}
data.frame(cb_lower, cb_upper) %>% 
  filter(cb_lower < 60 & cb_upper > 60) %>% 
  nrow/200
```
The expected proportion is $.95$, which is close. 
...
Answer: 
```{r}
gpa_new = gpa %>% filter(x %in% 20:30)
model = lm.fit_manual(gpa_new$x, gpa_new$y) 

ggplot(gpa, aes(x=x,y=y)) + geom_point(color = "deepskyblue4") + 
  geom_abline(intercept = model[1], slope = model[2], linetype = "dashed") + 
  labs(x = "ACT Score", y = "GPA", title = "Predicting GPA using ACT Scores") + 
  theme_minimal()
```
...
upper = c()
for(i in 20:30){
  conf_band = conf_band_reg(gpa_new, Xhat = i, alpha = .05)
  lower = c(lower, conf_band[1])
  upper = c(upper, conf_band[2])
}
conf_band_lower = data.frame(x = 20:30, y = lower)
conf_band_upper = data.frame(x = 20:30, y = upper)

ggplot(gpa, aes(x=x,y=y)) + geom_point(color = "deepskyblue4", alpha = .2) + 
  geom_line(data = conf_band_lower, color = "firebrick2") + 
  geom_line(data = conf_band_upper, color = "springgreen4") + 
  geom_abline(intercept = model[1], slope = model[2], linetype = "dashed") + 
  labs(x = "ACT Score", y = "GPA", title = "Predicting GPA using ACT Scores",
       subtitle = "with Confidence Band for ACT Scores between 20 and 30") + 
  theme_minimal()
```

The confidence band suggests that the true regression relation has been precisely estimated for where there is more data and less precise for where there is less data. 
...
Answer: 
```{r}
copier_new = copier %>% filter(x %in% 1:8)
model = lm.fit_manual(copier_new$x, copier_new$y) 

ggplot(copier, aes(x=x,y=y)) + geom_point(color = "deepskyblue4") + 
  geom_abline(intercept = model[1], slope = model[2], linetype = "dashed") + 
  labs(x = "Number of Copiers", y = "Service Time, in Minutes", 
       title = "Predicting Length of Service Time using Number of Available Copiers") + 
  theme_minimal()
```
...
upper = c()
for(i in 1:8){
  conf_band = conf_band_reg(copier_new, Xhat = i, alpha = .1)
  lower = c(lower, conf_band[1])
  upper = c(upper, conf_band[2])
}
conf_band_lower = data.frame(x = 1:8, y = lower)
conf_band_upper = data.frame(x = 1:8, y = upper)

ggplot(copier, aes(x=x,y=y)) + geom_point(color = "deepskyblue4", alpha = .2) + 
  geom_line(data = conf_band_lower, color = "firebrick2") + 
  geom_line(data = conf_band_upper, color = "springgreen4") + 
  geom_abline(intercept = model[1], slope = model[2], linetype = "dashed") + 
  labs(x = "Number of Copiers", y = "Service Time, in Minutes", 
       title = "Predicting Length of Service Time using Number of Available Copiers",
       subtitle = "with Confidence Band for Number of Copiers between 1 and 8") + 
  theme_minimal()
```

The confidence band suggests that the true regression relation has been precisely estimated for the entire region. 
