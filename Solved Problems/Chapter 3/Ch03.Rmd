...
gpa_resids %>% ggplot(aes(x = x, y = resid)) + 
  geom_point() + 
  labs(x = "ACT Scores", y = "Residuals", 
       title = "Dot Plot of the Residuals", subtitle = "of Regressing ACT Scores on GPA")
```

There does not seem to be a pattern in the residuals with the ACT scores.
...
brown.forsythe = function(df, split, alpha){
  ...
  t_ast = round((d1_bar - d2_bar) / (s  * sqrt((1/n1) + (1/n2))), 3)
  t = qt(alpha, n-2)
  
  if(abs(t_ast) > t){
    paste("At the alpha level of", alpha, "the test statistic is", t_ast, "and the null hypothesis is failed to be rejected. There is no sufficent evidence that the error variance is constant.", sep = ' ')
  }
  else{
    paste("At the alpha level of", alpha, "the test statistic is", t_ast, "and the null hypothesis is rejected. There is evidence that the error variance is constant.", sep = ' ')
  }
}
```
...
gpa_fulldata_resids %>% ggplot(aes(x = x2, y = resid)) + geom_point() + 
  labs(x = "Intelligence Test Score", y = "Residuals", 
       title = "Residuals against Intellligence Test Score")

gpa_fulldata_resids %>% ggplot(aes(x = x3, y = resid)) + geom_point() + 
